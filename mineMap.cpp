...
bool Mine::investigate() {
    ...
    if ((grid[temp.row][temp.col].rubble != temp.rubble) && tntGrid[r][c]) {
        ...
        if (r >= size || c >= size) {
            cerr << "out of bounds: (" << r << ", " << c << ")\n";
           exit(1);
        }

        grid[temp.row][temp.col].rubble = 0;
...
bool Mine::explode(Tile place) {
    ...
    while (!TNTq.empty()) {
        ...
        if (r < size) if (!tntGrid[r+1][c] && grid[r+1][c].rubble == -1) {
            TNTq.push(grid[r+1][c]);
            tntGrid[r+1][c] = true;
        } if (r != 0) if (!tntGrid[r-1][c] && grid[r-1][c].rubble == -1) {
            TNTq.push(grid[r-1][c]);
            tntGrid[r-1][c] = true;
        } if (c < size) if (!tntGrid[r][c+1] && grid[r][c+1].rubble == -1) {
            TNTq.push(grid[r][c+1]);
            tntGrid[r][c+1] = true;
        } if (c != 0) if (!tntGrid[r][c-1] && grid[r][c-1].rubble == -1) {
            TNTq.push(grid[r][c-1]);
            tntGrid[r][c-1] = true;
        }
        if (r < size) { 
...
void Mine::stats(Tile elt) {
    ...
    } else if (hardest.size() == N && cmp(hardest.back(),elt)) {
        hardest.pop_back();
        sort_insert(hardest, elt, false);
    } return;
}